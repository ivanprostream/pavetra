...
<form action="/admin/user_edit/{{ $user_data->id }}" method="POST" autocomplete="off">
    @csrf
    @method('PUT')
    <!-- Panel body -->
    <div class="p-6 space-y-6">
        <h2 class="text-2xl text-gray-800 font-bold mb-5">{{ __('admin.edit_user') }}</h2>
            ...
            <section>
                ...
                <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
                    <div class="sm:w-1/3">
                        ...
                        <input id="user_name" class="form-input w-full" name="name" type="text" placeholder="{{ __('admin.user_name') }}" value="{{ old('name', $user_data->name) }}" />
                        @error('name')
                    ...
                    <div class="sm:w-1/3">
                        ...
                        <input id="phone" class="form-input w-full" name="phone" type="text" placeholder="{{ __('admin.phone') }}" value="{{ old('phone', $user_data->phone) }}" />
                        @error('phone')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="sm:w-1/3">
                        ...
                        <select id="gender" class="form-select w-full" name="gender">
                            <option value="1" {{  $user_data->gender === 1 ? "selected" : "" }}>{{ __('admin.man') }}</option>
                            <option value="0" {{  $user_data->gender === 0 ? "selected" : "" }}>{{ __('admin.woman') }}</option>
                        </select>
                        @error('gender')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="sm:w-1/3">
                        ...
                        <select id="country" class="form-select w-full" name="country_id">
                            @foreach ($country_list as $item)
                                <option {{  $user_data->country_id === $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('country_id')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="sm:w-1/3">
                        ...
                        <select id="lang" class="form-select w-full" name="lang_id">
                            @foreach ($lang_list as $item)
                                <option {{  $user_data->lang_id === $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('lang_id')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <section>
                ...
                <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
                    <div class="sm:w-1/3">
                        ...
                        <input id="email" class="form-input w-full" name="email" type="email" placeholder="{{ __('admin.email_adress') }}" value="{{ old('email', $user_data->email) }}" />
                        @error('email')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="sm:w-1/3">
                        ...
                        <select id="role" class="form-select w-full" name="role">
                            @foreach ($roles as $item)
                                <option {{  $user_data->roles->first()->id === $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('role')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
