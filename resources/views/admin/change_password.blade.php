...
@section('main')
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
                ...
                <div>
                    ...
                    <ul class="flex flex-nowrap md:block mr-3 md:mr-0">
                        <li class="mr-0.5 md:mr-0 md:mb-0.5">
                            <a class="flex items-center px-2.5 py-2 rounded whitespace-nowrap" :class="settingsPanel === 'account' && 'bg-indigo-50'" href="{{ route('settings') }}">
                                ...
                                <span class="text-sm font-medium text-gray-600" :class="settingsPanel === 'account' ? 'text-indigo-500' : 'hover:text-gray-700'">{{ __('admin.my_account') }}</span>
                            </a>
                        </li>
                        <li class="mr-0.5 md:mr-0 md:mb-0.5">
                            <a class="flex items-center px-2.5 py-2 rounded whitespace-nowrap bg-indigo-50" href="/admin/change_password">
                                ...
                                <span class="text-sm font-medium text-indigo-500">{{ __('admin.change_password') }}</span>
                            </a>
                        </li>
            ...
            <!-- Panel -->
            <div class="grow">
                <form action="/admin/change_password/{{ $user->id }}" method="POST">
                    @csrf
                    @method('PUT')
                    <!-- Panel body -->
                        ...
                        <h2 class="text-2xl text-gray-800 font-bold mb-5">{{ __('admin.my_account') }}</h2>
                            ...
                            <section>
                                ...
                                <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-5">
                                    <div class="sm:w-1/3">
                                        <input id="password" class="form-input w-full" name="password" type="password" placeholder="{{ __('admin.change_password_text') }}" value="" />
                                        @error('password')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="sm:w-1/3">
                                        <input id="password_confirmation" class="form-input w-full" name="password_confirmation" type="password" placeholder="{{ __('admin.repeat_password_text') }}" value="" />
